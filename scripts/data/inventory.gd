...
func perform_transaction(
	...
	money += money_amount
	if item_type != Inventory.ItemType.NONE:
		_add_item(item, item_type)


func _add_item(item: Item, item_type: Inventory.ItemType) -> void:
	match item_type:
		Inventory.ItemType.WEPON:
			_add_weapon(item)
		Inventory.ItemType.ARMOR:
			_add_armor(item)
		Inventory.ItemType.ARTIFACT:
			_add_artifact(item)
		Inventory.ItemType.COLLECTABLE:
			_add_collectable(item)
		Inventory.ItemType.CONSUMABLE:
