...
func load_level(level_path: String):
	var current_scene := GameManager.scene_manager.scene_container_node
...
func load_party_overworld(pc_paths: Dictionary, members_path: Array):
	...
	GameManager.party_manager.overworld_party = []
	...
	for pm in overworld_party:
		...
		GameManager.party_manager.overworld_party.append(party_member)
		party_member.position = pos

func load_combat_party(combat_party: Array):
	GameManager.party_manager.combat_party = []
	for pm in combat_party:
		var party_member: CombatantData = load(pm["path"])
		GameManager.party_manager.combat_party.append(party_member)
...
func load_money(money: int):
	GameManager.inventory.money = money

func load_weapons(weapon_data: Array):
	GameManager.inventory.weapons = []
...
func load_consumables(consumables_data: Array):
	...
	for stack in consumables_data:
		...
		GameManager.inventory.weapons.append(consumable_stack)
...
func serialize_state_json() -> Dictionary:
	var serialized_data := {
		"globals": {
			...
			"inventory": {
				"money": GameManager.inventory.money,
				"wepons": [],
				"armors": [],
				"collectables": [],
				"consumables": []
		...
		"current_scene": {
			"level": GameManager.scene_manager.scene_container_node.get_child(0).scene_file_path
		},
		"party_container": {
...
func serialize_combat_party_data() -> Array:
	return GameManager.party_manager.combat_party.map(func(pm) -> Dictionary:
		return {
...
func serialize_overworld_party() -> Array:
	return GameManager.party_manager.overworld_party.map(func(pm) -> String: return pm.scene_file_path)

func serialize_weapons() -> Array:
	return GameManager.inventory.weapons.map(func(stack) -> Dictionary:
		return {
...
func serialize_armors() -> Array:
	return GameManager.inventory.armors.map(func(stack) -> Dictionary:
		return {
			"path": stack.item.resource_path,
			"display_name": stack.item.display_name,
			"texture_path": stack.item.texture.resource_path,
...
func serialize_collectables() -> Array:
	return GameManager.inventory.collectables.map(func(stack) -> Dictionary:
		return {
			"path": stack.item.resource_path,
			"display_name": stack.item.display_name,
			"texture_path": stack.item.texture.resource_path,
...
func serialize_consumables() -> Array:
	return GameManager.inventory.consumables.map(func(stack) -> Dictionary:
		return {
			"path": stack.item.resource_path,
			"display_name": stack.item.display_name,
			"texture_path": stack.item.texture.resource_path,
